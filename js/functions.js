...
export const Questions = [
  ...
  },
  {
    ...
    a: [
      ...
      { text: "Making decisions breaks my mind", keywords: ["deep"] },
    ],
  },
  {
    id: 3,
...
const assignToList = (like) => {
  const gameCard = document.getElementById("gameCard");
  let list;
  if (like) {
    list = document.getElementById("liked");
  } else {
    list = document.getElementById("disliked");
  }
  list.append(gameCard.lastElementChild);
  let last = Array.from(document.querySelectorAll(".gameId")).pop();
  document.getElementById("likeInput").value = last.textContent;
};

const swipeCard = (like) => {
  const gameCard = document.getElementById("gameCard");
  if (like) {
  ...
  setTimeout(assignToList, 50, like);
};
