        ...
        fetch(`${API_URL}`)
          ...
          .then((data) => {
            data.results.forEach(function (game) {
              console.log(game);
              gameCard.innerHTML += `<li class="gameSlide" style="background-color: #${game.dominant_color}"><h2>${game.name}</h2><div class="img"><img src="${game.background_image}"></div><div class="like"></div>
              <div class="dislike"></div></li>`;
              $("#gameCardWrapper").jTinder(options);
            });
          });
      };
